...
// echo $_SESSION['registro'];
			...
			if($contColas==0){
			//echo "error_imp";
			echo "<script>alert('usuario no autorizado');window.parent.location.href='index.php'</script>";
			}
...
$permiso_modifPrecio=substr($row['p1'],24,1);
...
$strSQLC="select * from sucursal where cod_suc='".$_SESSION['user_sucursal']."' ";
$resultadoC=mysql_query($strSQLC,$cn);
$row=mysql_fetch_array($resultadoC);
$percep_suc=$row['percepcion'];
...
//if(document.getElementById('productos').style.visibility=='visible'){
	...
	//	alert(document.getElementById('tblproductos').rows.length);
		//document.getElementById('tblproductos').rows[i].focus();
		...
		//alert('');
	     ...
	     // document.formulario.codprod.focus();	
		//move_up();
		//document.getElementById("detalle").style.overflow.scrollTo(0,1150);	
		...
		objDiv.scrollTop = objDiv.scrollTop+50;
			...
			//$("#detalle").animate({ scrollLeft: "+=5" }, "slow");
			
		//capa_desplazar = jQuery('#detalle');
		//alert(capa_desplazar.innerHTML);
		//capa_desplazar.scrollTo(0, capa_desplazar.getSize().scroll.y + 50);
			...
			//}else{
				...
				document.formulario.ter.value=0;			*/
						...
						if(document.formulario.cantidad.value!="" && document.formulario.codprod.value!=""){
						  ...
						  if(document.formulario.cantidad.value==0 && (document.formulario.punit.value!="" || document.formulario.punit.value!=0) && !document.formulario.precio.disabled ){	   
						   //alert();
					...
					//Cantidad de sub unidad
							...
							if(parseFloat(saldo) >= parseFloat(cant) || p1=='N' || sd=="N"){ //Nuevo sd(Control Stock Detalle)
								...
								if(document.formulario.permiso_modifPrecio.value=='N'){
...
//		var datos = window.open("empresa.php?total_doc="+total_doc+'&moneda_doc='+tmoneda+'&tcambio_doc='+tcambio+'&sucursal='+sucursal+'&tienda='+tienda+'&responsable='+responsable+'&incluidoigv='+incluidoigv+'&impto='+impto+'&impuesto1='+impuesto1+'&baseimp='+baseimp,"","dialogWidth:600px;dialogHeight:530px,top=180,left=200,status=yes,scrollbars=yes");
		...
		if(!isset(datos)){	
		var codcabOE=document.formulario.codcabOE.value;		
		datos = window.open("empresa2.php?total_doc="+total_doc+'&moneda_doc='+tmoneda+'&tcambio_doc='+tcambio+'&sucursal='+sucursal+'&tienda='+tienda+'&responsable='+responsable+'&incluidoigv='+incluidoigv+'&impto='+impto+'&impuesto1='+impuesto1+'&baseimp='+baseimp+'&codcabOE='+codcabOE+'&percepcion='+percepcion+'&totalProdPercep='+totalProdPercep,"ventana","width=600px height=450px,top=180,left=200,status=yes,scrollbars=yes");
...
//alert(texto);
	...
	doAjax('compras/det_aux.php','&clasificacion=1&nomb_det='+valor+'&temp='+temp+'&tipomov='+tipomov+'&tienda='+tienda+'&criterio='+temp_criterio+'&moneda_doc='+moneda_doc+'&tipoBus='+controltipoBus[1]+'&cambiarPrecio='+cambiarPrecio,'detalle_prod','get','0','1','','');
...
<div id="alerta" style="z-index:999; position:absolute; left: 267px; top: 149px; width: 332px; display:none">
  ...
  <table width="800" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
      ...
      <td height="28">&nbsp;</td>
      <td><span class="Estilo47">Empresa:</span></td>
      ...
      <td colspan="8"><fieldset>
        <table width="644" border="0" cellpadding="0" cellspacing="0">
		  ...
		  function mostrar_factor(texto){
            ...
            <td width="75">
			<!--onBlur="javascript:this.value=parseFloat(this.value)"-->
			<input name="cantidad"  type="text" size="7"    onKeyUp="calc_pre_total()"  onKeyDown="prev_validarNumero(this,event)" />
            <script>
            ...
            <td width="71"><input id="punit"  name="punit" type="text" size="7" onKeyUp="calcular_ptotal()"  onKeyDown="prev_validarNumero2(this,event,'00')" disabled="disabled" /></td>			
            <td width="79">
			<!--
			<input name="precio" type="text" size="7" onKeyUp="calcular_cant()" />
			-->
            <input  name="precio" type="text" size="7" onKeyUp="calcular_cant()" disabled="disabled"/>
  ...
  <div id="divOrder" style="position:absolute; left: 534px; top: 207px; height: 48px; width: 95px; display:none; z-index:9999">
...
//alert(pos);
	...
	var permiso16='N';
	//alert(permiso16);
	if(precio_nuevo<precosto && permiso16=='N'){
	alert('El precio no puede ser menor a el precio de venta5');
...
/*
function recalcular_cant(control,producto,e,precosto,mon_prod,cant_actual){
...
/*
function prev_validarNumero2(control,e){
...
//alert(e.keyCode);
	try{
	//alert(e.keyCode);
		if((e.keyCode>=96 && e.keyCode<=105) || (e.keyCode>46 && e.keyCode<58) || e.keyCode==8 || e.keyCode==190 || e.keyCode==37 || e.keyCode==39 || e.keyCode==110|| e.keyCode==9){
			temp=control.value.split(".");
			if(e.keyCode==190 || e.keyCode==110){
				if(temp[1]!=undefined){	
					e.keyCode=0;
...
<embed style="visibility:hidden" src="compras/alerrta1.mp3" id="sound2" width="0" heigh="0" autostart="false" enablejavascript="true"/>
