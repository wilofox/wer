<?php
	...
	session_start();
	$tipo = 2;
	$titu = "Gerencia :: Contabilidad :: Libro de Ventas";
...
<style type="text/css">
	...
	.anulado  {text-decoration:line-through;font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 9px; color:#990000; }
	-->
</style>
...
<form name="form1" method="post" action="">
      <table width="800" height="386" border="0" cellpadding="0" cellspacing="0">
        <tr style="background:url(imagenes/white-top-bottom.gif)">
          <td width="764" height="22" colspan="6" align="left" style="border:#999999"><span class="Estilo34 ">
            <?php echo $titu; ?>
			...
			<input name="coduser"  type="hidden" value="<?php echo $coduser; ?>" />
          </span></td>
        </tr>
        <tr >
    ...
    Calendar.setup({
	 ...
	 for (i=0;i<document.form1.turno.options.length;i++)
            ...
            if (document.form1.turno.options[i].value==valor1)
               ...
               document.form1.turno.options[i].selected=true;
				  ...
				  <input <?php if($_REQUEST['tickets']!=''){?> checked="checked" <?php }?> style="border: 0px; background-color:#F9F9F9; " type="checkbox" name="tickets" id="tickets" value="checkbox">
					...
					<?php }else{?>
					 <input  checked="checked"  style="border: 0px; background-color:#F9F9F9; visibility:hidden;display:none " type="checkbox" name="tickets" id="tickets" value="checkbox" >
        ...
        <tr>
		  ...
		  <div id="detalle" style="width:760px; height:300px; overflow:scroll">		  </div>		  </td>
        </tr>
        <tr>
          <td  colspan="6" valign="top">
		   <div id="paginacion" style="width:760px; height:20px;">		  </div>		  </td>
        </tr>
  </table>
...
var temp1=0;
 ...
 var tipdoc = document.getElementsByName("tipdoc");
	...
	if(tip_doc=="C"){
		var tip_rep="reportes/det_rpt_lventas_excel.php";
	}else{
		var tip_rep="reportes/det_rpt_lventas_sunat_excel.php";
	}

vent=window.open(tip_rep+'?fecha='+fecha+'&fecha2='+fecha2+'&docRk='+docRk+'&cod_suc='+sucursal+'&cod_tienda='+almacen+'&codigo_usuario='+vendedor+'&codigo_turno='+turno+'&codigo_caja='+serie+'&tickets='+tickets+'&tipo='+tipo+'&coduser='+coduser);
...
function cargar_detalle(pagina){
	...
	if(tip_doc=="C"){
		var det_doc="reportes/det_rpt_lventas.php";
	}else{
		var det_doc="reportes/det_rpt_lventas_sunat.php";
	}

//alert(det_doc);
	doAjax(det_doc,'fecha='+fecha+'&fecha2='+fecha2+'&docRk='+docRk+'&cod_suc='+sucursal+'&cod_tienda='+almacen+'&codigo_usuario='+vendedor+'&codigo_turno='+turno+'&codigo_caja='+serie+'&tickets='+tickets+'&tipo='+tipo+'&pagina='+pagina+'&coduser='+coduser,'view_det','post','0','1','','');
...
function listaprod(texto){
	...
	function Guarda(){
		...
		doAjax('reportes/documentos.php','&docRk='+docRk+"&reporte=LIBRO_VENTAS",'','get','0','1','','');
...
function validartecla2(e,valor,temp){
	if(document.getElementById(temp).style.visibility!='visible' ){
		document.form1.carga.value="S";
		var tipo=document.form1.tipo.value;
		doAjax('reportes/documentos.php','&temp='+temp+'&tipo='+tipo+"&reporte=LIBRO_VENTAS",'listadocumentos','get','0','1','','');
	}
}
