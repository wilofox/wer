...
function listaprod(texto){
	...
	//alert(temp_criterio);
	doAjax('../compras/det_aux.php','&clasificacion=1&nomb_det='+valor+'&temp='+temp+'&tipomov='+tipomov+'&tienda='+tienda+'&criterio='+temp_criterio+'&prov_asoc='+document.formulario.prov_asoc.value+'&moneda_doc='+moneda_doc,'detalle_prod','get','0','1','','');
...
<body  onload="iniciar();carga_div()"   onUnload="vaciar_sesiones()" >
...
<form id="formulario" name="formulario" method="post" action="">
  <table width="807" border="0" cellpadding="0" cellspacing="0">
     ...
     <tr style="background:url(../imagenes/white-top-bottom.gif)">
       ...
       <span class="Estilo14"><span class="Estilo114">
       <input style="text-align:center" disabled="disabled" type="hidden" name="idAreaCosto" id="idAreaCosto" maxlength="2" size="2" >
       <input type="hidden" name="nomAreaCosto" id="nomAreaCosto"  onKeyUp="cambiar_chofer('AC')" onFocus="cambiar_chofer('AC')" size="13" maxlength="50">
       </span></span></td>
    </tr>
...
$k++;
    ...
    Calendar.setup({
        ...
        <tr>
          ...
          <td align="center"><strong><span style="font:Verdana, Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300">ORDEN DE TRABAJO</span></strong>				  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
          <td class="Estilo14">
          ...
          <td class="Estilo14">F.Termino</td>
          <td align="left"><input disabled="disabled" name="fven" type="text" size="15" maxlength="10"  value="<?php echo date('d-m-Y')?>"  onKeyUp="generar_ceros(event,'0',this)" onChange="enfocar_fecha(this)">
...
-->
 ...
 <div id="lisDocApro" style="border:#238CE2 solid 1px; background:#E2FAFE; position:absolute; left:274px; top:139px; width:505px; height:284px; z-index:2; visibility:hidden; background-color: #F1FBFE;">
 
 <table width="506" height="289" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td height="26" style="background:url(../imagenes/aqua-hd-bg.gif); color:#FFFFFF">&nbsp;</td>
    <td height="26" style="background:url(../imagenes/aqua-hd-bg.gif); color:#000000;"><strong>PRESUPUESTOS APROBADOS</strong></td>
    <td height="26" align="center" style="background:url(../imagenes/aqua-hd-bg.gif); color:#000000;  color:#000000; text-decoration:underline; cursor:pointer" onClick="salir();"><strong>x</strong></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<div style="overflow-y:scroll; height:200px">
	  <table width="458" height="44" border="0" cellpadding="0" cellspacing="0">
        <tr style="background:url(../imagenes/bg_contentbase2.gif); background-position:100px 60px;">
          <td width="25" height="23" style="color:#FFFFFF"></td>
          <td width="69" style="color:#FFFFFF"><strong>F.Emision</strong></td>
          <td width="82" style="color:#FFFFFF"><strong>F.Aprobado</strong></td>
          <td width="181" style="color:#FFFFFF"><strong>Cliente</strong></td>
          <td width="93" style="color:#FFFFFF"><strong>N&ordm;Presupuesto</strong></td>
        </tr>
        <?php 
	  $strSQLPre="select * from cab_mov where cod_ope='PO' and estadoOT='T' and flag_r='' ";
	  $resultadoPre=mysql_query($strSQLPre,$cn);
	  $i=0;
	  while($rowPre=mysql_fetch_array($resultadoPre)){
	  ...
	  ?>
        <tr bgcolor="<?php echo $backlgrond ?>" >
          <td height="21" style="color:#333333; border-bottom:#E5E5E5 solid 1px"><input style="background:none; border:none" name="radioPO" id="radioPO" type="radio" value="<?php echo $rowPre['cod_cab']."|".$rowPre['serie']."|".$rowPre['Num_doc']."|".$rowPre['sucursal']?>" onClick="selPOref(this)"></td>
          <td style="color:#333333; border-bottom:#E5E5E5 solid 1px"><?php echo substr($rowPre['fecha'],0,10) ?></td>
          <td style="color:#333333; border-bottom:#E5E5E5 solid 1px"><?php echo substr($rowPre['f_venc'],0,10) ?></td>
          <td style="color:#333333; border-bottom:#E5E5E5 solid 1px">
		  <?php 
		  $strSQLClie="select * from cliente where codcliente='".$rowPre['cliente']."'";
		  $resultadoClie=mysql_query($strSQLClie,$cn);
		  $rowClie=mysql_fetch_array($resultadoClie);
		  echo $rowClie['razonsocial']; 
		  
		  ?>
		  </td>
          <td style="color:#222222; border-bottom:#E5E5E5 solid 1px"><?php echo $rowPre['serie']."-".$rowPre['Num_doc'] ?></td>
        </tr>
        <?php $i++; }?>
      </table>
	</div>	</td>
  ...
  <tr>
    <td width="12">&nbsp;</td>
    <td height="43" align="center"><input type="hidden" name="codCabRefPO" id="codCabRefPO">
    <input type="button" name="Submit5" value="  Referenciar Presupuesto  " onClick="referenciarPO()"></td>
    </tr>
  <tr>
	...
	function rpta_bus_numero(texto){
		...
		if(document.getElementById("capaCopiarDoc").style.visibility=="visible"){
		   ...
		   // document.formulario.auxiliar.disabled=false;
			...
			//alert();
			document.getElementById('lisDocApro').style.visibility="visible";
...
function seleccionar_cbo(control,valor){
	...
	*/
	...
	var impto=document.formulario.impto.value;
		...
		document.formulario.condicion.disabled=false;
		document.formulario.responsable.focus();
		document.getElementById("lisDocApro").style.visibility="hidden";
...
function cargar_cbo_doc(){
	...
	function filtros(){
		...
		doAjax('../compras/det_aux.php','&clasificacion=1&nomb_det='+valor+'&temp='+temp+'&tipomov='+tipomov+'&tienda='+tienda+'&criterio='+temp_criterio+'&prov_asoc='+document.formulario.prov_asoc.value+'&comboclasificacion='+comboclasificacion+'&categoria='+categoria+'&subcategoria='+subcategoria+'&moneda_doc='+moneda_doc,'detalle_prod','get','0','1','','');
...
function referenciarPO(){

	var tempDatos=document.formulario.codCabRefPO.value.split("|");
	var serie=tempDatos[1];
	var numero=tempDatos[2];
	var sucursal=tempDatos[3];
	
    doAjax('../peticion_ajax.php','&serie='+serie+'&numero='+numero+'&doc=PO&sucursal='+sucursal+'&det_ref'+'&accion=buscar','rpta_det_ref','get','0','1','',''); 
}
...
//	cargar_ref(serie,numero,cod_cli_ref,des_cli_ref,cod_cab_ref,moneda_doc,impto);
	var tempDatos=document.formulario.codCabRefPO.value.split("|");
	var serie=tempDatos[1];
	var numero=tempDatos[2];
	//var sucursal=tempDatos[3];

	cargar_ref(serie,numero,temp[2],temp[3],temp[1],temp[4],temp[5]);
}


function selPOref(obj){
document.formulario.codCabRefPO.value=obj.value;
}
