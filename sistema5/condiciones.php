...
session_start();
 ...
 unset($_SESSION['array_des'][0]);
 unset($_SESSION['array_des'][1]);
 unset($_SESSION['array_des'][2]);
...
include('conex_inicial.php');
		...
		$strSQL="select * from detope where  documento='$codigo' order by id";
		//echo $strSQL;
		$resultado=mysql_query($strSQL,$cn);
		while($row=mysql_fetch_array($resultado)){
		
         $_SESSION['array_des'][0][] = $row['condicion'];
		 //$_SESSION['array_des'][1][] = $row['descondi'];
		 list($nombreCondi)=mysql_fetch_row(mysql_query("SELECT nombre FROM condicion WHERE codigo = '".$row['condicion']."'",$cn));
//echo "SELECT nombre FROM condicion WHERE codigo = '".$row['descondi']."'";
		 $_SESSION['array_des'][1][] = $nombreCondi;
...
<form id="form1" name="form1" method="post" action="" >
  <table  width="546" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td width="14">&nbsp;</td>
      <td><span class="Estilo1">Condiciones</span></td>
      <td>&nbsp;</td>
	  ...
	  <?php 
	   ...
	   $filtro=" where 1 ";
		...
		$filtro.=" and codigo!='".$_SESSION['array_des'][0][$subkey]."' ";
				...
				//echo "select * from condicion ".$filtro." order by nombre ";
	  
	  ?>
	 	  
	  
	  <select name="lst_condi" size="10"  style="width:170">
        <?php 
		
		
	  	  
	    $resultados1 = mysql_query("select * from condicion ".$filtro." order by nombre ",$cn); 
	  ...
	  <div id='cond_doc' style="height:150px; border:#999999 solid 1px; vertical-align:top; overflow:auto">
	   <table id="lista_condi" width="270"  border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
			<tr>
			  ...
			  <td width="184" bgcolor="#1FAF9D"><span class="Estilo20">Condiciones</span></td>
			  <td width="31" bgcolor="#1FAF9D"><span class="Estilo20">Deuda</span></td>
		    </tr>
			...
			<tr onClick="entrada(this)" bgcolor="#FFFFFF">
			  ...
			  <td><input <?php echo $marcar ?> type="checkbox" name="deuda[]" id="deuda" value="checkbox" style="border:#FFFFFF solid 1px"></td>
		    </tr>
			<?php 
			  }
			}
...
<script>
	...
	doAjax('peticion_ajax.php','add&condicion='+condicion+'&descondi='+descondi,'add_cond','get','0','1','','');
	...
	function quitar_item(valor){
			...
			for (var i=0;i<document.form1.radiobutton.length;i++){ 
			   if (document.form1.radiobutton[i].checked) {
				...
				if(navigator.appName == 'Microsoft Internet Explorer'){
					...
					var descripcion = document.form1.radiobutton[i].parentNode.parentNode.childNodes[3].innerHTML;
				}
		...
		//aBorrar.parentNode.removeChild(aBorrar);											
		doAjax('peticion_ajax.php','add&delete&condicion='+codigo,'add_cond','get','0','1','','');
		//alert(codigo);
	}
	...
	function save_cond(){
		...
		//alert(deuda);
		doAjax('peticion_ajax.php','save&cod_docu='+document.form1.cod_docu.value+'&deuda='+deuda,'cerrar','get','0','1','','');
