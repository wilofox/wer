...
//echo $codDoc;
	...
	if(isset($_REQUEST['numero'])){
		...
		//---------------------documento-----------------------------------------------------------
		  $strSQL_doc="select * from operacion where codigo='".$_REQUEST['documento']."' and  tipo='1' ";
	      ...
	      $monto=0;
		  $impuesto1=0;
		  $totalDoc=$_REQUEST['importe'];
		  }
		  if($_REQUEST['impuestos']==1){
		  $incluidoigv="S";
		  $monto=number_format($_REQUEST['importe']/(($impto/100)+1),2,'.','');
		  $impuesto1=$_REQUEST['importe']-$monto;
		  $totalDoc=$_REQUEST['importe'];
		  }
			
		  if($_REQUEST['impuestos']==2){
		  $incluidoigv="N";
  	      $monto=$_REQUEST['importe'];
		  $impuesto1=$_REQUEST['importe']*($impto/100);
		  $totalDoc=$monto+$impuesto1;
		  }		   
		  
		  //$impto=$row_doc['imp1'];
		//-----------------------------------------------------------------------------------------
		$strSQL3="insert into cab_mov(cod_cab,tipo,cod_ope,Num_doc,serie,cod_vendedor,caja,cliente,ruc,fecha,f_venc,moneda,impto1,tc,b_imp,igv,servicio,percepcion,total,saldo,tienda,sucursal,flag,flag_r,motivo,noperacion,items,condicion,incluidoigv,obs1,obs2,obs3,obs4,obs5,inafecto,fecha_aud,pc,kardex,deuda,transportista,chofer,dirPartida,dirDestino)values('".$codigo."','1','".$_REQUEST['documento']."','".$_REQUEST['numero']."','".$_REQUEST['serie']."','".$_REQUEST['responsable']."','".$caja."','".$_REQUEST['codcliente']."','".$ruc."','".cambiarfecha($_REQUEST['fechaDoc'])."','".cambiarfecha($_REQUEST['fechaCaja'])."','".$_REQUEST['monedaDoc']."','".$impto."','".$_SESSION['tc']."','".$monto."','".$impuesto1."','".$servicio."','".$percepcion."','".$totalDoc."','".$saldo."','".$_REQUEST['tiendaOT']."','".$_REQUEST['sucOT']."','".$flag."','RA','".$motivo."','".$noperacion."','".$items."','1','".$incluidoigv."','".$obs1."','".$obs2."','".$obs3."','".$obs4."','".$obs5."','".$permiso4."','".$fecha_aud."','".$_SESSION['pc_ingreso']."','N','".$deuda."','".$transportista."','".$chofer."','".$dirPartida."','".$dirDestino."')";		
		//echo $strSQL3;
	...
	//-------------------------------------pagos--------------------------------------------------------
				...
				$strSQ_pagos3="insert into pagos(id,t_pago,numero,fecha,fechav,monto,moneda,fechap,tcambio,referencia,pc,cod_user) values ('".$id_pagos."',1,'CASH','".cambiarfecha($_REQUEST['fechaDoc'])."','".cambiarfecha($_REQUEST['fechaCaja'])."',".$totalDoc.",'".$_REQUEST['monedaDoc']."','".$fecha_aud."',".$_SESSION['tc'].",'".$codigo."','".$_SESSION['pc_ingreso']."','".$_SESSION['codvendedor']."')";
				//echo $strSQ3;
				mysql_query($strSQ_pagos3,$cn);
	
	//--------------------------------------------------------------------------------------------------	
				
	$tempOper=explode("-",$_REQUEST['costooperativo']);
	$costoOperativo=$tempOper[0];
	list($nom_costoOpe)=mysql_fetch_row(mysql_query("select nombre from costoperativo where id='".$tempOper[0]."'",$cn));
	
	$strSQLdet="insert into det_mov(cod_cab,tipo,cod_ope,serie,numero,auxiliar,tienda,sucursal,cod_prod,codanex,nom_prod,tcambio,moneda,precio,cantidad,imp_item,fechad,descargo,afectoigv) values('".$codigo."','1','".$_REQUEST['documento']."','".$_REQUEST['serie']."','".$_REQUEST['numero']."','".$_REQUEST['codcliente']."','".$_REQUEST['tiendaOT']."','".$_REQUEST['sucOT']."','TEXTO','".$costoOperativo."','".$nom_costoOpe."','".$_SESSION['tc']."','".$_REQUEST['monedaDoc']."','".$_REQUEST['importe']."','1','".$_REQUEST['importe']."','".cambiarfecha($_REQUEST['fechaDoc'])."','N','N')";
	
	mysql_query($strSQLdet,$cn);
		
	$strSQL4="insert into referencia(cod_cab,serie,correlativo,cod_cab_ref) values('".$codigo."','".$_REQUEST['serieOT']."','".$_REQUEST['numeroOT']."','".$_REQUEST['codcabOT']."')";
	...
	$strSQL3="insert into tempdoc(id,sucursal,tipodoc,doc,serie,numero,auxiliar,estado,usuario)values('".$id."','".$_REQUEST['sucOT']."','1','".$_REQUEST['documento']."','".$_REQUEST['serie']."','".$_REQUEST['numero']."','".$_REQUEST['codcliente']."','G','".$_SESSION['codvendedor']."')";
	mysql_query($strSQL3,$cn);	
	
	header("location: cajaGastos.php?CodDoc=".$codDoc);
  ...
  <table width="756" height="365" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td width="734" align="center" bgcolor="#E6E6E6"><span class="Estilo1">Caja Gastos Operativos 
        ...
        <input type="hidden" name="sucOT" id="sucOT" value="<?php echo $sucursalOT ?>">
		<input type="hidden" name="tiendaOT" id="tiendaOT" value="<?php echo $tiendaOT ?>">
        <input type="hidden" name="codcabOT" id="codcabOT" value="<?php echo $codcabOT ?>">
      </span></td>
      ...
      <td align="left"><fieldset>
        ...
        <table width="598" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <input  readonly="readonly" name="numeroOT" id="numeroOT" type="text" size="8" maxlength="7" value="<?php echo $numeroOT; ?>"></td>
            ...
            <td width="159" valign="bottom"><span class="Estilo41">Moneda: </span>
			  <span class="Estilo50">
              <?php if($monedaOT=='01')echo " SOLES (S/.) "; else echo " DOLARES (US$.) " ?>
              <input name="monedaOT" type="hidden" id="monedaOT" value="<?php echo $monedaOT ?>" size="10" maxlength="10" readonly="readonly">
			  </span></td>
          </tr>
          <tr>
            ...
            <tr>
              <td colspan="3"><table width="717" height="30" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td width="150"><input readonly="readonly" name="impPres" id="impPres" type="text" class="Estilo35" size="10" style="text-align:right"></td>
                  <td width="56"><span class="Estilo49">Utilizado:</span></td>
                  <td width="165"><input readonly="readonly" name="impUtil" id="impUtil" type="text" class="Estilo35" size="10" style="text-align:right"></td>
                  <td width="69"><span class="Estilo49">Disponible:</span></td>
                  <td width="131"><input readonly="readonly" name="impDisp" id="impDisp" type="text" class="Estilo35" size="10" style="text-align:right"></td>
                </tr>
              </table></td>
            </tr>
          </table>
      ...
      <td height="136"><fieldset ><legend class="Estilo42">DOCUMENTO DE INGRESO</legend>
		  ...
		  ?>
            ...
            <tr>
              ...
              <td class="Estilo41"><input name="descliente" id="descliente" type="text"  onFocus="cambiar_chofer('prov')"></td>
            ...
            <tr>
              ...
              <td class="Estilo41"><span class="Estilo48">
                <select name="impuestos">
                  <option value="1">Incluido IGV</option>
				 <!-- <option value="2">No Incluido IGV</option>-->
...
doAjax('../peticion_ajax2.php','&peticion=ConsCostoO&costoOpe='+costoOpe+'&CodDocOT='+CodDocOT,'rptacambiarCostoO','get','0','1','','');
}
...
document.form1.impUtil.value=texto;
document.form1.impDisp.value=parseFloat(document.form1.impPres.value)-texto;
...
document.form1.valor_chofer.focus();
