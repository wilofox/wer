...
//echo "entro";
 $accion=$_REQUEST['accion'];
				...
				$surtidor=$_REQUEST['surtidor'];
				$factor=$_REQUEST['factor'];
				
				
				
	 if($accion=='n'){
		$clase->new_manguera($id,$nombre,$descripcion,$tanque,$surtidor,$factor);
	}

	if($accion=='e'){
	//echo  $_REQUEST['codigo'];
		$clase->act_manguera($id,$nombre,$descripcion,$tanque,$surtidor,$factor);
	}
	if($accion=='el' && isset($_REQUEST['codigo'])){
	//echo $_REQUEST['cod'];
		$clase->elim_manguera($_REQUEST['codigo']);
	}
...
event.keyCode=0;
	event.returnValue=false;
//	alert("m");
 doAjax('new_manguera.php','accion=n','nuevo_suc','get','0','1','','');
...
<?php
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" >
       ...
       <td height="27" colspan="13" style="border:#999999"><span class="Estilo34">Mantenimientos :: <span class="Estilo14 Estilo38">Manguera </span></span></td>
    </tr>
    <tr>
        ...
        <tr>
		  ...
		  //obj.cells[3].style.backgroundImage="";
		      ...
		      <table title="Nuevo[F3]" width="80" height="21" border="0" cellpadding="0" cellspacing="0">
                <tr onMouseOver="entrar_btn(this);" onMouseOut="salir_btn(this)" style="cursor:pointer" onClick="javascript: doAjax('new_manguera.php','accion=n','nuevo_suc','get','0','1','','');" >
                  <td width="1" ></td>
      ...
      <td colspan="7"><form name="formbuscar" method="post" action="?" onSubmit="return false"><table width="329" border="0" align="center">
          <tr>
            ...
            <select name="tipo" id="tipo">
              ...
              <option value="nom_mang" selected>Nombre</option>
              <option value="cod_tanques">Cod. Tanque</option>
			  <option value="cod_surtidor">Cod. Surtidor</option>
              </select>
            </label></td>
            <td width="18"><label>
...
doAjax('new_manguera.php','accion=e&cod='+cod,'nuevo_suc','get','0','1','','');
//}
}
function eliminar(cod){

	if(confirm('Esta seguro que desea eliminar ?')){
	location.href='manguera.php?accion=el&codigo='+cod;
	//this.form1.submit();
	}
...
doAjax('lista_manguera.php','condicion='+cond+'&texto='+texto+'&pag='+pag,'view_det','get','0','1','','');
