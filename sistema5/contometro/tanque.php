...
//echo "entro";
 $accion=$_REQUEST['accion'];
	 ...
	 if($accion=='n'){
		$clase->new_tanque($id,$nombre,$descripcion,$producto,$capacidad,$ubicacion,$largo,$radio);
	}

	if($accion=='e'){
	//echo  $_REQUEST['codigo'];
		$clase->act_tanque($id,$nombre,$descripcion,$producto,$capacidad,$ubicacion,$largo,$radio);
	}
	if($accion=='el' && isset($_REQUEST['codigo'])){
	//echo $_REQUEST['cod'];
		$clase->elim_tanque($_REQUEST['codigo']);
	}
...
function ocultar(){
document.getElementById("tipGLP").style.visibility="hidden";
...
event.keyCode=0;
	event.returnValue=false;
//	alert("m");
 doAjax('new_tanque.php','accion=n','nuevo_suc','get','0','1','','');
...
<?php
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" >
       ...
       <td height="27" colspan="13" style="border:#999999"><span class="Estilo34">Mantenimientos :: <span class="Estilo14 Estilo38">Tanques </span></span></td>
    </tr>
    <tr>
        ...
        <tr>
		  ...
		  //obj.cells[3].style.backgroundImage="";
		      ...
		      <table title="Nuevo[F3]" width="80" height="21" border="0" cellpadding="0" cellspacing="0">
                <tr onMouseOver="entrar_btn(this);" onMouseOut="salir_btn(this)" style="cursor:pointer" onClick="javascript: doAjax('new_tanque.php','accion=n','nuevo_suc','get','0','1','','');" >
                  <td width="1" ></td>
      ...
      <td colspan="7"><form name="formbuscar" method="post" action="?" onSubmit="return false"><table width="329" border="0" align="center">
          <tr>
            ...
            <select name="tipo" id="tipo">
              ...
              <option value="nom_tanq" selected>Nombre</option>
              <option value="cod_prod">Cod. Prod.</option>
              </select>
            </label></td>
            <td width="18"><label>
...
doAjax('new_tanque.php','accion=e&cod='+cod,'nuevo_suc','get','0','1','','');
//}
}
function eliminar(cod){

	if(confirm('Esta seguro que desea eliminar ?')){
	location.href='tanque.php?accion=el&codigo='+cod;
	//this.form1.submit();
	}
...
doAjax('lista_tanque.php','condicion='+cond+'&texto='+texto+'&pag='+pag,'view_det','get','0','1','','');
...
function tipotanque(obj){
//producto
	obj=document.form1.producto;
	var indice = obj.selectedIndex 
	var prod =obj.options[indice].text;
	prod=prod.toUpperCase();
	prod=prod.substr(0,3);	

if (document.form1.accion.value!='e'){
	document.form1.largo.value='';
	document.form1.radio.value='';
}
document.getElementById("tipGLP").style.visibility="visible";
if (prod=='GLP'){
	if (document.form1.accion.value!='e'){
		document.form1.largo.value='0';
		document.form1.radio.value='0';
	}
document.getElementById("tipGLP").style.visibility="hidden";
}
