...
//echo "entro";
 $accion=$_REQUEST['accion'];
				...
				$idunit=$_REQUEST['codigo'];
				$simbolo=$_REQUEST['simbolo'];
				$nombre=$_REQUEST['nombre'];
				$factorSub=$_REQUEST['factorSub'];
				
				
	 if($accion=='n'){
		$clase->new_medidas($idunit,$simbolo,$nombre,$factorSub);
	}

	if($accion=='e'){
	//echo  $_REQUEST['codigo'];
		$clase->act_medidas($idunit,$simbolo,$nombre,$factorSub);
	}
	if($accion=='el' && isset($_REQUEST['cod'])){
	//echo $_REQUEST['cod'];
		$clase->elim_medidas($_REQUEST['cod']);
	}
...
event.keyCode=0;
	event.returnValue=false;
//	alert("m");
 doAjax('new_unidad.php','accion=n','nuevo_suc','get','0','1','','');
...
<?php
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" >
       ...
       <td height="27" colspan="13" style="border:#999999">&nbsp;<span class="Estilo34">Administraci&oacute;n :: Art&iacute;culos :: U. de Medida </span></td>
    </tr>
	<tr>
      <td colspan="7"><table width="99%" height="21" border="0" cellpadding="0" cellspacing="0" style="border-bottom:#CCCCCC solid 1px">
        <tr>
          ...
          <td width="76" ><table title="Nuevo[F3]" width="85" height="21" border="0" cellpadding="0" cellspacing="0">
              <tr onMouseOver="entrar_btn(this);" onMouseOut="salir_btn(this)" style="cursor:pointer" onClick="javascript: doAjax('new_unidad.php','accion=n','nuevo_suc','get','0','1','','');" >
                <td width="1" ></td>
...
doAjax('new_unidad.php','accion=e&cod='+cod,'nuevo_suc','get','0','1','','');
}
}
...
function eliminar(cod){
	...
	location.href='m_unitmedida.php?accion=el&cod='+cod;
	//this.form1.submit();
	}
...
doAjax('listmedidas.php','condicion='+cond+'&texto='+texto+'&pag='+pag,'view_det','get','0','1','','');
