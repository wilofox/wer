...
<table id="fila" width="356" border="0" cellpadding="0" cellspacing="0" >
  ...
  <tr>
	...
	<table width="450" height="26%" border="0" cellpadding="0" cellspacing="1" bordercolor="#FFFFFF" id="<?php echo $tabla?>">
	 ...
	 <?php 
		...
		$criterio=$_REQUEST['criterio'];
			...
			/*if($_REQUEST['tipomov']==1 && $_REQUEST['temp']=='auxiliares'){
				 ...
				 $strSQL1="select * from cliente where baja='N' and tipo_aux='P' ".$filtro. " )";
				 				 
				 }else{
				 $strSQL1="select * from cliente where baja='N' and tipo_aux='P' and $criterio like '%$nombre%' order by $criterio limit 50";
				 }
			}else{
			...
			if($_REQUEST['tipomov']==2 && $_REQUEST['temp']=='auxiliares'){
			 $strSQL1="select * from cliente  where baja='N' and tipo_aux='C' and $criterio like '%$nombre%'   order by $criterio limit 50";
			}else{
			...
			$codigosistema=str_pad($nombre, 6, "0", STR_PAD_LEFT);  
				...
				$strSQL1="select * from producto where cod_prod='$nombre' ".$filtro3." limit 100 ";
				}else{
				 if($criterio=="idproducto"){
				 $strSQL1="select * from producto where idproducto='$codigosistema' ".$filtro3." limit 100 ";
				   }else{
						if($criterio=="serie"){
						$strSQL1="select * from series,producto where tienda='".$tienda."' and idproducto=producto and serie='$nombre' and estado='F' ".$filtro3." limit 100 ";
						}else{
						$strSQL1="select * from producto where nombre like '%$nombre%' ".$filtro3." order by nombre limit 100 ";
						}
				   }
				}
  ...
  while($row1=mysql_fetch_array($resultado1)){
        ...
        <tr bgcolor="#FFFFFF">
          ...
          <td width="55%"><font color="#000000" face="Verdana, Arial, Helvetica, sans-serif"  style="font-size:10px; font:bold"><?php
		   ...
		   echo $descripcion;
		   ?></font><a name="ancla<?php echo $i;?>" id="ancla"></a></td>
        </tr>
