...
//echo "entro";
 $accion=$_REQUEST['accion'];
				...
				$idcondicion=$_REQUEST['codigo'];
				$nombre=$_REQUEST['nombre'];
				$csunat=$_REQUEST['csunat'];
					
	 if($accion=='n'){
		$clase->new_condicion($idcondicion,$nombre,$csunat);
	}

	if($accion=='e'){
	//echo  $_REQUEST['codigo'];
		$clase->act_condicion($idcondicion,$nombre,$csunat);
	}
	if($accion=='el' && isset($_REQUEST['cod'])){
	//echo $_REQUEST['cod'];
		$clase->elim_condicion($_REQUEST['cod']);
	}
...
event.keyCode=0;
	event.returnValue=false;
//	alert("m");
 doAjax('new_condicion.php','accion=n','nuevo_suc','get','0','1','','');
...
<?php
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" >
       ...
       <td height="27" colspan="13" style="border:#999999">&nbsp;<span class="Estilo34">Administraci&oacute;n :: Documentos :: <span class="Estilo14 Estilo38">Condiciones</span></span></td>
    </tr>
	<tr>
      <td colspan="7"><table width="99%" height="21" border="0" cellpadding="0" cellspacing="0" style="border-bottom:#CCCCCC solid 1px">
        <tr>
          ...
          <td width="76" ><table title="Nuevo[F3]" width="94" height="21" border="0" cellpadding="0" cellspacing="0">
              <tr onMouseOver="entrar_btn(this);" onMouseOut="salir_btn(this)" style="cursor:pointer" onClick="javascript: doAjax('new_condicion.php','accion=n','nuevo_suc','get','0','1','','');" >
                <td width="1" ></td>
      ...
      <td colspan="7"><form name="formbuscar" method="post" action="?" onSubmit="return false"><table width="329" border="0" align="center">
          <tr>
            ...
            <select name="tipo" id="tipo">
              <option value="codsunat">C.Sunat</option>
...
function editar(cod){
//alert(cod);
	if(confirm('Esta seguro que desea editar esta condicion puede tener productos relacionados')){
doAjax('new_condicion.php','accion=e&cod='+cod,'nuevo_suc','get','0','1','','');
}
}

function eliminar(cod){

	if(confirm('Esta seguro que desea eliminar esta condicion?')){
	location.href='m_condicion.php?accion=el&cod='+cod;
	//this.form1.submit();
	}
...
doAjax('listcondicion.php','condicion='+cond+'&texto='+texto+'&pag='+pag,'view_det','get','0','1','','');
