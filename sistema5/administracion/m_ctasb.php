...
//echo "entro";
 $accion=$_REQUEST['accion'];
    ...
    switch($accion){
        case 'n':$clase->new_ctasb($id,$titular,$banco,$moneda,$cta,$sucursal);break;
        case 'e':$clase->act_ctasb($id,$titular,$banco,$moneda,$cta,$sucursal);break;
        case 'el':$clase->elim_ctasb($id);break;
    }
...
evt.keyCode=0;
	evt.returnValue=false;
//	alert("m");
 doAjax('new_ctasb.php','accion=n','nuevo_suc','get','0','1','','');
...
<?php
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" >
    <tr style="background:url(../imagenes/white-top-bottom.gif)">
       <td height="27" colspan="13" style="border:#999999">&nbsp;<span class="Estilo34">Administraci&oacute;n :: Finanzas :: <span class="Estilo14 Estilo38">Cuentas Bancarias</span></span></td>
    </tr>
	<tr>
      <td colspan="7"><table width="99%" height="21" border="0" cellpadding="0" cellspacing="0" style="border-bottom:#CCCCCC solid 1px">
        <tr>
          ...
          <td width="76" ><table title="Nuevo[F3]" width="102" height="21" border="0" cellpadding="0" cellspacing="0">
              <tr onMouseOver="entrar_btn(this);" onMouseOut="salir_btn(this)" style="cursor:pointer" onClick="javascript: doAjax('new_ctasb.php','accion=n','nuevo_suc','get','0','1','','');" >
                <td width="1" ></td>
    ...
    <tr>
      <td colspan="7"><form name="formbuscar" method="post" action="?" onSubmit="return false"><table width="329" border="0" align="center">
          ...
          <input name="tempauxprod" type="hidden" id="tempauxprod" value="productos"></form>		  
		  </td>
    </tr>
...
function editar(cod){
//alert(cod);
	if(confirm('Esta seguro que desea editar esta Cuenta puede tener movimientos')){
doAjax('new_ctasb.php','accion=e&cod='+cod,'nuevo_suc','get','0','1','','');
}
}
...
function eliminar(cod){
	...
	location.href='m_ctasb.php?accion=el&codigo='+cod;
	//this.form1.submit();
	}
	...
	if((e.keyCode>=48 && e.keyCode<=57) || e.keyCode==8 || e.keyCode==46 || e.keyCode==37 || e.keyCode==39){
		...
		if(nc[1]!=undefined){
			...
			for(var j=0;j<te.length;j++){
				if (j==3 || j==11 || j==12 ) {
					te2=te2+te.substring((np-1),j)+"-";
					np=j+1;
				}
			}
			objNpt.value=te2+te.substring((np-1),j+1);
			
		}
		if (n.length==3 || n.length==12 || n.length==14 ) {
			objNpt.value=n+"-";
		}
	}else{
		var nc=n.split("");
...
elem.style.backgroundColor='';
    //alert(document.form1.elements[a].name+" - "+document.form1.elements[a].value);
	if(document.form1.elements[a].value=="" || document.form1.elements[a].value=="-"){
...
doAjax('listctasb.php','tipo='+cond+'&texto='+texto+'&pag='+pag,'view_det','get','0','1','','');
