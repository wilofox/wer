...
<?
	...
	$resultados = mysql_query("SELECT * FROM det_mov DM ".$SqlConWerRK." and DM.tipo='2' GROUP BY cod_prod" ,$cn);
	...
	$total_registros = mysql_num_rows($resultados); 
 	$strSQL = "
		...
		FROM det_mov DM 
		INNER JOIN producto P ON DM.cod_prod = P.idproducto	".$SqlConWerRK."  and DM.tipo='2' GROUP BY DM.cod_prod ".$order."  LIMIT ".$inicio.",".$registros." ";
	$j = 0;
...
<?
	$resultados = mysql_query("select * from det_mov DM  $SqlConWerRK and DM.tipo='2' " ,$cn);
	$total_registros = mysql_num_rows($resultados); 
	//,DM.cod_prod as cod_prod
 	$strSQL="select *,DM.moneda as moneda2, DM.precio as precio2 from det_mov DM 	
	INNER JOIN producto P ON DM.cod_prod = P.idproducto		
	 $SqlConWerRK and DM.tipo='2'  $order LIMIT $inicio, $registros";
	//echo $strSQL;
	$j=0;
	$resultado=mysql_query($strSQL,$cn);
...
<?
	$resultados = mysql_query("select * from det_mov DM  $SqlConWerRK and DM.tipo='2' group by auxiliar" ,$cn);
	$total_registros = mysql_num_rows($resultados); 
	...
	INNER JOIN producto P ON DM.cod_prod = P.idproducto	
	$SqlConWerRK and DM.tipo='2' group by auxiliar $order LIMIT $inicio, $registros";
	//echo $strSQL;
	$j=0;
	$resultado=mysql_query($strSQL,$cn);
