...
if($sucursal==0){
	...
	$saldos=saldo_ant($fecha1,$codigo,$tipo,$filtro_multi);
	$temp=explode("?",$saldos);
	//print_r($temp);
	$saldo_ant_s=$temp[0];
	$saldo_ant_d=$temp[1];
...
function saldo_ant($fecha1,$cliente,$tip,$where=""){
	include('../conex_inicial.php');
	...
	$strSQL332="Select * from cab_mov where cliente='".$cliente."' and tipo='$tip' and substr(fecha,1,10) < '".$fecha1."' ".$where;
	//echo $strSQL332;
	...
	while($row332=mysql_fetch_array($consulta332)){
		...
		$moneda_doc=$row332['moneda'];
		$strSQL333="Select p.tipo,p.moneda,p.monto,p.tcambio from pagos p where p.estado!='A' and p.fecha < '".$fecha1."' and p.referencia='".$row332['cod_cab']."' ";
		$consulta32=mysql_query($strSQL333,$cn);
		$numrow=mysql_num_rows($consulta32);
		//echo "<br>".$row332['cod_ope']." ".$row332['serie']."-".$row332['Num_doc']."=====>".$row332['moneda']."  ".number_format($row332['total'],2,'.','')."+".number_format($row332['percepcion'],2,'.','')."<br>";
		//echo "-------------PAGOS-----------------------------------<br>";
		while($row=mysql_fetch_array($consulta32)){
			if($moneda_doc!=$row['moneda']){
				switch($row['moneda']){
					case '01':$montox=number_format($row['monto'],2,'.','')/$row['tcambio'];break;
					case '02':$montox=number_format($row['monto'],2,'.','')*$row['tcambio'];break;
				}
			...
			//echo $row['monto']." ".$row['tcambio']."-".$row['moneda']."========>".$row['tipo']." ".number_format($montox,2,'.','')."<br>";
		}
	}
	//echo "////////////".$cargos_s."//////////////".$cargos_d."<br>";
	//$strSQL333="Select p.tipo,p.moneda,p.monto,p.tcambio,c.moneda as monedadoc from pagos p inner join cab_mov c on c.cod_cab=p.referencia where p.estado!='A' and p.fecha < '".$fecha1."' and c.cliente='".$cliente."' and c.tipo='$tip' and substr(c.fecha,1,10) < '".$fecha1."' and c.flag!='A' and c.cod_ope NOT IN ('NC','TN') and c.deuda='S' ";
	$strSQL333="Select p.tipo,p.moneda,p.monto,p.tcambio,c.moneda as monedadoc from pagos p inner join cab_mov c on c.cod_cab=p.referencia where p.estado!='A' and p.fecha < '".$fecha1."' and p.referencia in (Select cod_cab from cab_mov where cliente='".$cliente."' and tipo='$tip' and substr(fecha,1,10) < '".$fecha1."' ".$where.") ";
	// )p.referencia in (Select cod_cab from cab_mov  where  ";
	//echo "<br>".$strSQL333."<br>";

	
	//}
	//echo number_format($abonos_s,2,'.','')."///////////////".number_format($abonos_d,2,'.','');
