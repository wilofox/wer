...
if(isset($_REQUEST['excel'])){
	...
	switch($_REQUEST['tipo']){
		case '1':$titulo="PAGAR";break;
		case '2':$titulo="COBRAR";break;
	}
}
...
<?php 
if(isset($_REQUEST['excel'])){
	echo "<center><b><h1>REPORTE DE ESTADO DE CUENTAS POR ".$titulo."</h1></b></center><table border='0'>";
...
<table width="896" border="0" cellpadding="1" cellspacing="1" >
	...
	//echo $_REQUEST['tickets']."---".$_REQUEST['tickets2']."----".$_REQUEST['doc_cancel']."----".$_REQUEST['doc_cancel2']."<br>"; 
	...
	*/
	...
	if($agcliente=="true"){	
		$strSQL_cab="select *,razonsocial  from cab_mov inner join cliente on cab_mov.cliente=cliente.codcliente where tipo='$tipo'  and substring(fecha,1,10) < '$fecha1' and cab_mov.cod_ope!='NC' and cab_mov.cod_ope!='TN' ".$filtro1.$filtro2.$filtro3.$filtro4.$filtro5.$filtro6." and flag!='A' order by razonsocial asc,f_venc asc";
		$res_cab=mysql_query($strSQL_cab,$cn);
		$cod_ant_cab="";
		while($rowxx=mysql_fetch_array($res_cab)){
			$listasa=saldoa($rowxx['cod_cab'],$fecha1,$rowxx['moneda'],$rowxx['total'],"1");
			if($listasa>0){
				//echo $rowxx['cod_ope']." ".$rowxx['serie']."-".$rowxx['Num_doc']."<br>";
	...
	//$strSQL="select distinct(cliente) from cab_mov where tipo='$tipo' and substring(fecha,1,10) between '$fecha1' and '$fecha2' ".$filtro1.$filtro2.$filtro3.$filtro4.$filtro5.$filtro6." order by f_venc asc";
		...
		//echo "('".$cod_ant_cab."')";
		$strSQL="select * from cab_mov where (tipo='$tipo' and substring(fecha,1,10) between '$fecha1' and '$fecha2' and cab_mov.cod_ope!='NC' and cab_mov.cod_ope!='TN' ".$filtro1.$filtro2.$filtro3.$filtro4.$filtro5.$filtro6."  and flag!='A') or cod_cab in ('".$cod_ant_cab."')  order by f_venc asc";
		//echo $strSQL;
	}		
	$registros = 25; 
	...
	$tgd=0;	
	//$GLOBALS['tactas']="150";
	function muestra_datos($datos,$ag="",$tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$filtro6){	
		...
		while($row=mysql_fetch_array($datosx)){
			...
			$listasa=saldoa($referencia,$fecha1,$row['moneda'],$row['total'],"1");
			$total=$listasa;
			
			list($cliente)=mysql_fetch_array(mysql_query("select razonsocial from cliente where codcliente='".$row['cliente']."'"));
			if($ag=="s" && $row['cliente']!=$temcli && $temcli!=""){
				$tot=mostrar_total($tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$filtro6,$temcli);
				if($x>1 ){//&& $tregistro==$x 
					//$mos=$tot;
					//echo $mos;
					echo $tot;
			...
			if($flag!='A'){
				...
				*/
				...
				$estilo="";
				//echo "<br>///////////<br>".$_REQUEST['tickets2']."<br>///////////<br>";
				if($_REQUEST['tickets2']=="S"){
					$f2=$fecha2;
	...
	<tr bgcolor="#F9F9F9" <?php echo $estilo;?> onclick="entrada(this)">
		...
		<td align="right" ><span class="Estilo10">
				<?php 
					if($_REQUEST['tickets2']!="S"){
						$sql2="Select * from pagos where referencia='".$row['cod_cab']."' and fecha between '$fecha1' and '$fecha2'";
						...
						while($row22=mysql_fetch_array($con2)){
							...
							if($row22['tipo']=="A"){
								...
								switch($row['moneda']){
									...
									case '01':$tactas=$tactas+number_format($acta2,2,'.','');$acta=$acta+number_format($acta2,2,'.','');break;
									//case '02':$GLOBALS['tactad']=$GLOBALS['tactad']+number_format($acta2,2,'.','');$acta=$acta+number_format($acta2,2,'.','');break;
									case '02':$tactad=$tactad+number_format($acta2,2,'.','');$acta=$acta+number_format($acta2,2,'.','');break;
								}
							}
						}
	...
	<tr>
		...
		if($ag=="s" && 	$codcliente==""){	
			...
			//echo $tipo."--".$fecha1."--".$fecha2."--".$filtro1."--".$filtro2."--".$filtro3."--".$filtro4."--".$filtro5."--".$filtro6."--".$temcli."<br>";
			$temtotal="<tr><td colspan='12'>".mostrar_total($tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$filtro6,$temcli)."</td></tr>";
			//echo $temtotal;
		}
	}
	...
	if($agcliente=="true"){	
		...
		muestra_datos($textsql,"s",$tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$filtro6);
	//$total_registros=$tot;
	}else{
		muestra_datos($textsql,"",$tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$filtro6);
	}
...
<?php 
	function saldoa($referencia,$fecha2,$mone,$tot,$tip){
		...
		$tot_car=0;
		if($tip=="1"){
		//echo $referencia."----".$fecha2."----".$mone."----".$tot."-----".$tip."<br>";
		}
