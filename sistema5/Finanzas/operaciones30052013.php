...
switch($modulo){
	...
	default:
		...
		switch($accion){
		  ...
		  $consulta="SELECT ca.moneda as moneda,concat(ca.tienda,'-',ti.des_tienda) as tienda, ca.cod_cab AS cab, ca.cod_ope AS td, CONCAT(ca.serie,'-',ca.num_doc) AS documento, ca.fecha AS fec_doc, ca.f_venc AS fec_vcto, if(ca.saldo<ca.total,ca.saldo,ca.total) AS mtotal, ca.saldo AS msaldo FROM cab_mov ca INNER JOIN tienda ti on ti.cod_tienda =ca.tienda INNER JOIN operacion ope ON ope.codigo=ca.cod_ope WHERE saldo>0.00 AND SUBSTR(ope.p1,5,1)='S' and ope.tipo='".$tipo."' and codigo!='TB' and codigo!='TF' and codigo!='TS' and ca.cliente='".$cliente."'".$filtro;
		...
		?>
		  <tr>
            ...
            <td><span class="Estilo11"><input type="checkbox"<?php //echo $dis; ?>value="<?php echo $row2['cab']; ?>" name="chkok" id="chkok" onClick="<?php echo $oper[0];//$marc; ?>" <?php echo $mc; ?>></span></td>
      ...
      <?php
		    ...
		    <tr>
				...
				$tot=$tot+$subt;
				?></font>
				<td width="10" bgcolor="#33FFCC" style="border-color:#09C"><img onclick="EliminarDoc('<?php echo $_SESSION['pagos'][0][$subkey];?>')" src="../imgenes/eliminar.png" /></td>
		    </tr>
		    <?php
				}
			?>
			...
			<?php
				foreach ($_SESSION['pagos'][0] as $subkey=> $subvalue) {
					...
					<tr>
						...
						$tot=$tot+$subt;
						?></font>
						<td width="10" bgcolor="#33FFCC" style="border-color:#09C"><img onclick="EliminarDoc('<?php echo $_SESSION['pagos'][0][$subkey];?>')" src="../imgenes/eliminar.png" />
				    </tr>
			    <?php
					}
				}
...
<table width="350" border="0">
					...
					<tr>
						...
						<td width="56"><input style="font-size:9px" onkeyup="RecalcularLetra(this,event,'<?php echo $subkey;?>')" size="8" height="12" type="text" name="monto[]" id="monto[]" value="<?php echo $_SESSION['pagoslet'][3][$subkey];?>"><?php $tot=$_SESSION['pagoslet'][3][$subkey]+$tot;?></font>
