...
if(isset($_REQUEST['excel'])){
	...
	if($_REQUEST['almacen']!="" && $_REQUEST['almacen']!="0"){
		...
		echo  "<tr><td><b>Tienda:</b></td><td>".$dat['des_tienda']."</td></tr>";
	}
	if($_REQUEST['vendedor']!="" && $_REQUEST['vendedor']!="000"){
		...
		echo  "<tr><td><b>Vendedor:</b></td><td>".$dat['usuario']."</td></tr>";
	}
	...
	*/
	...
	function muestra_datos($datos,$ag=""){	
		...
		while($row=mysql_fetch_array($datosx)){
			...
			if($ag=="s" && $row['cliente']!=$temcli ){
				...
				$tot=mostrar_total($GLOBALS['tipo'],$GLOBALS['fecha1'],$GLOBALS['fecha2'],$GLOBALS['filtro1'],$GLOBALS['filtro2'],$GLOBALS['filtro3'],$GLOBALS['filtro4'],$GLOBALS['filtro5'],$row['cliente']);
				$temp="";
				if($_REQUEST['tipo']=='1'){
					$tempCP=" Proveedor: ";
				}else{
	...
	<tr>
		...
		if($ag=="s" && 	$GLOBALS['codcliente']==""){
			$temtotal="<tr><td colspan='12'>".mostrar_total($GLOBALS['tipo'],$GLOBALS['fecha1'],$GLOBALS['fecha2'],$GLOBALS['filtro1'],$GLOBALS['filtro2'],$GLOBALS['filtro3'],$GLOBALS['filtro4'],$GLOBALS['filtro5'],$temcli)."</td></tr>";
			echo $temtotal;
		}
	}
...
<?php 
	...
	function mostrar_total($tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$where=""){
		if($where!=""){
		...
		if($tipo=='2'){
			$strSQL="select sum(total) as total,count(cod_cab) as cantidad from cab_mov where tipo='$tipo' and flag!='A' and substring(fecha,1,10) between '$fecha1'  and '$fecha2' ".$filtro1.$filtro2.$filtro3." and flag!='A' and cod_ope='TB' and cod_ope!='TN' and cod_ope!='NC' ".$filtro5.$filtrox." order by cod_cab ";
			...
			$strSQL="select sum(total) as total,count(cod_cab) as cantidad from cab_mov where tipo='$tipo' and flag!='A' and substring(fecha,1,10) between '$fecha1' and '$fecha2' ".$filtro1.$filtro2.$filtro3." and flag!='A' and cod_ope!='TN' and cod_ope!='NC' and cod_ope='TF' ".$filtro5.$filtrox." order by cod_cab ";//" and cod_ope='TF' "
			//echo $strSQL;
			$resultado=mysql_query($strSQL);
			$row=mysql_fetch_array($resultado);
		...
		$dattable.='<td bgcolor="#F9F9F9" colspan="2" align="center"><span class="Estilo7 Estilo33">Total ';
		...
		$dattable.=' <td colspan="2" bgcolor="#F9F9F9" align="center"><span class="Estilo7 Estilo33">Total ';
		if($where==""){ $dattable.= "General";}
		...
		$dattable.='</span></td>
			...
			$dattable.='</td><td align="right" bgcolor="#F9F9F9" class="Estilo7 Estilo34">';
			$dattable.=number_format($row_tot['totald'],2,'.','')-number_format($total_ab_d,2,'.','');
			$dattable.='</td>
		</tr>';
		//$dattable.='</table>';
	...
	if($GLOBALS['total_paginas']==$GLOBALS['pagina']){
		echo mostrar_total($tipo,$fecha1,$fecha2,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$codcliente);
	}
	if(!isset($_REQUEST['excel'])){
	?>
    |
