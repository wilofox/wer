  ...
  <?php 
		...
		//------------------------------------------
			...
			if($cliente!="" && $cliente!="T" ){
				$filtrocli=" and cliente='".$_REQUEST['cliente']."'";
			}
...
/* $SQLConsulta=" where $almacen1
 ...
 if($Estado!="P"){
 	...
 	$SQLConsulta.=" substring(fecha,1,10) between '".formatofecha($fec1)."' and '".formatofecha($fec2)."'".$SQLEstado.$Cmoneda.$filtrocli;
 }else{
	 $SQLConsulta="$SQLEstado$Cmoneda$filtrocli";
 }

$AnRK="";

$resultados = mysql_query("select * from multicj CM
inner join cliente Cl on CM.cliente =Cl.codcliente 
$SQLConsulta
order by numcje" ,$cn);
$total_registros = mysql_num_rows($resultados); 
			
/*$strSQL="select * from multicj CM
inner join cliente Cl on CM.cliente =Cl.codcliente 	
...
<tr bgcolor="<?php echo $color_row?>" onClick="entrada(this);" ondblclick="doc_det(this)" >
	<td width="33" align="center" ><input style="border: 0px; background:none; " type="radio" name="XDato" value="<?php  echo $row['multi_id']?>" /><b><?=$j;?></b></td>
	...
	<? //} ?></td>
	<td width="69" class="texto1" align="center" ><?php echo extraefecha($row['fecha']);?></td>
	<td width="85" class="texto1" ><?php echo $row['PC']; ?></td>
	<td width="81" class="texto1"><?php echo $row['numcje']; ?></td>			   
	...
	<td width="76" align="right" class="texto1"><?php 
		$resultados11 = mysql_query("select * from usuarios where codigo='".$row['codvendedor']."' ",$cn); 
		$rowSM=mysql_fetch_array($resultados11);
		echo $rowSM['usuario'];
	?></td>
	<td width="100" align="right" class="texto1"><?php echo $row['canlet'];?></td>
