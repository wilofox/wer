...
<table width="896" border="0" cellpadding="1" cellspacing="1" >
	...
	*/
	...
	//$strSQL="select distinct(cliente) from cab_mov where tipo='$tipo' and substring(fecha,1,10) between '$fecha1' and '$fecha2' ".$filtro1.$filtro2.$filtro3.$filtro4.$filtro5.$filtro6." order by f_venc asc";
		if($_REQUEST['tickets2']=="S" && $_REQUEST['doc_cancel']=="N"){
			$strSQL_cab="select *,razonsocial  from cab_mov inner join cliente on cab_mov.cliente=cliente.codcliente where tipo='$tipo'  and substring(fecha,1,10) <= '$fecha2' and cab_mov.cod_ope!='NC' and cab_mov.cod_ope!='TN' ".$filtro1.$filtro2.$filtro3.$filtro4.$filtro5.$filtro6." and flag!='A' order by razonsocial asc,f_venc asc";
			...
			$strSQL="select *,razonsocial  from cab_mov inner join cliente on cab_mov.cliente=cliente.codcliente where cod_cab in ('".$cod_ant_cab."') order by razonsocial asc,f_venc asc";
		}else{
		...
		if($_REQUEST['tickets2']=="S" && $_REQUEST['doc_cancel']=="N"){
			$strSQL_cab="select * from cab_mov where tipo='$tipo' and substring(fecha,1,10) <= '$fecha2' and cab_mov.cod_ope!='NC' and cab_mov.cod_ope!='TN' ".$filtro1.$filtro2.$filtro3.$filtro4.$filtro5.$filtro6." and flag!='A' order by f_venc asc";
			$res=mysql_query($strSQL_cab,$cn);
			$cod_ant_cab="";
			while($rowxx=mysql_fetch_array($res)){
				$listasa=saldoa($rowxx['cod_cab'],$fecha1,$rowxx['moneda'],$rowxx['total'],"1");
				echo $listasa.">0 && ".$_REQUEST['doc_cancel2']."==S && ".$_REQUEST['doc_cancel']."==N O ".$listasa."==0 && ".$_REQUEST['doc_cancel2']."==N && ".$_REQUEST['doc_cancel']."==S<br>";
				if(($listasa>0 && $_REQUEST['doc_cancel2']=="S" && $_REQUEST['doc_cancel']=="N") || ($listasa==0 && $_REQUEST['doc_cancel2']=="N" && $_REQUEST['doc_cancel']=="S")){
					//echo $rowxx['cod_ope']." ".$rowxx['serie']."-".$rowxx['Num_doc']."<br>";
					$cod_ant_cab.="','".$rowxx['cod_cab'];
				}
			}
			$strSQL="select * from cab_mov where cod_cab in ('".$cod_ant_cab."') order by f_venc asc";
		}else{
