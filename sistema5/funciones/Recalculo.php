<?php 
function Recalculo($codigo,$fechak,$filtro,$cn=CONEXION){
...
function saldo_anterior($fechad,$producto,$sucursal,$codcab,$cn=CONEXION){	
...
///kardex fisico
function recalculo2($codigo,$fechak,$filtro,$tipo,$sucursal,$archivo=NULL,$cn=CONEXION){
	//if($fechak!=date('Y-m-d')){	
		$costo_inven_ant=0;
		$saldo_ant=0;
		$tienda="";
		...
		switch($tipo){
			case '1':$filtro= " and c.sucursal='".$sucursal."' and substring(d.fechad,1,10)<='".$fechak."' ";break;
			case '2':$filtro= " and c.sucursal='".$sucursal."' and substring(d.fechad,1,10)<'".$fechak."' "; break;
			case '3':$filtrof=" and substring(d.fechad,1,10)<'".$fechak."' ";break; 
			case '4':$filtrof=" and substring(d.fechad,1,10)<='".$fechak."' ";break;
		}	
		$sql_kardex=mysql_query("select idproducto from producto where idproducto='".$codigo."' LIMIT 1",$cn) or die(mysql_error());  
		
		$row_kardex=mysql_fetch_array($sql_kardex,MYSQL_ASSOC);
		if($row_kardex[0]=="N"){
		...
		$j=0;
		//while($row10=mysql_fetch_array($resultado)){
		while($row10=mysql_fetch_array($resultadox,MYSQL_ASSOC)){
...
function recalculo3($codigo,$fechak,$fechak2,$filtro,$tipo,$sucursal,$cn=CONEXION){
	//if($fechak!=date('Y-m-d')){
		$costo_inven_ant=0;
		$saldo_ant=0;
		$tienda="";
...
function recalculoActCost($codigo,$fechak,$filtro,$tipo,$sucursal,$cn=CONEXION){
	//if($fechak!=date('Y-m-d')){
	$costo_inven_ant=0;
	$saldo_ant=0;
	$tienda="";
