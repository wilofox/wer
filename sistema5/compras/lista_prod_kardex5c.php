...
<table id="lista_productos" width="720" height="20" border="0" cellpadding="0" cellspacing="0" >
  <tr bgcolor="<?php echo $color_row?>" onclick="entrada(this)" onDblClick="detalle_prod('<?php echo $row['idproducto']?>')">
    ...
    <td class="bordeCelda texto1" width="282" align="left" valign="top" ><?php echo caracteres(substr($row['nombre'],0,38))?></td>
	...
	if ($rowUni['id']==""){		 
		...
		if ($rowUni['mconv']==""){		 
			...
			//$Unidad =$row['factor']-(10-$Valor[1]);
				...
				$Unidad=('0.'.$Valor[1])*$row['factor'];
				//echo $row['factor'];
				
			if ($Valor[1]>0){
				echo ' &#124; '.number_format($Unidad,2,'.','')." und" ;				
			}			
