...
<div id="detalle" style="width:850px; height:164px; overflow:auto;border:0px solid;" >
<table width="852" height="24" border="0" cellpadding="0" cellspacing="0" id="lista_productos">
  <?php 
...
$SQLConsulta and substring(fecha,1,10) between '".$inifec[2]."-".$inifec[1]."-".$inifec[0]."' and '".$finfec[2]."-".$finfec[1]."-".$finfec[0]."' $filtroOrden " ,$cn);
...
?>
  ...
  <tr bgcolor="<?php echo $color_row?>" onClick="entrada(this);Anular('S<?=$AnRK;?>')" ondblclick="doc_det(this)"  >
      <td width="26" align="center"><input onclick="Anular('S<?=$AnRK;?>')" style="border: 0px; background:none; " type="radio" name="XDato" value="<?php  echo $row['cod_cab']?>" />
	  ...
	  <input type="hidden" name="tipformato" value="<?php echo $data ?>" />	  </td>
             <td width="119" class="texto1"><?php echo $row['fecha']?></td>	
              ...
              <td width="148" class="texto1"><?php echo utf8_encode($row['razonsocial'])?></td>
			  ...
			  if ($row['monedaSD']=='02'){echo 'US$.';}else{echo 'S/.';}
			   ...
			   ?></td>
              <td width="68" align="right" class="texto1"><?php 
	...
	echo $row['total'];
			  ...
			  ?></td>
			  <td width="130" align="right" class="texto1">
			  <?php 
	...
	echo formatofecha(substr($rowR['fecha'],0,10)).'-|-'.$rowR['Num_doc'];		
			   ...
			   ?></td>			  
			  <td width="91" align="center" class="texto1"><?php echo $row['tienda']?></td>	
