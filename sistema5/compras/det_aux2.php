...
$filtroKardex="";
if($codigoDoc=='PO'){
//$filtroKardex=" and kardex='S' ";
}

$perm_chkElemProd=$_REQUEST['chkElemProd'];
$perm_ElemProd=$_REQUEST['ElemProd'];

//echo $_REQUEST['predefecto'];
	...
	<table width="570" height="26%" border="0" cellpadding="0" cellspacing="1" bordercolor="#FFFFFF" id="<?php echo $tabla?>">
	 ...
	 <?php 
			...
			$codigosistema=str_pad($nombre, 6, "0", STR_PAD_LEFT);  
				...
				$strSQL1="select * from producto where (upper(cod_prod) like '%".strtoupper($nombre)."%' or upper(codanex2) like '%".strtoupper($nombre)."%' or upper(codanex3) like '%".strtoupper($nombre)."%' ) ".$filtro3.$listaProd.$filtroKardex." limit 100 ";
					
				}else{
					
				 if($criterio=="idproducto"){
				 $strSQL1="select * from producto where idproducto='$codigosistema' ".$filtro3.$listaProd.$filtroKardex." limit 100 ";
				   }else{
						if($criterio=="serie"){
						if ($estSP=='V'){
						...
						$strSQL1="select * from series,producto where tienda='".$tienda."' and idproducto=producto and upper(serie)='".strtoupper($nombre)."' ".$filtroEst." ".$filtro3." limit 100 ";
						}else{
							 if($tipoBus=="aprox"){
						...
						$strSQL1="select * from producto where ".$filtroBus." ".$filtro3.$listaProd.$filtroKardex." order by nombre limit 100 ";
						}
				   }
...
$i=0;
  ...
  //echo $perm_chkElemProd;
	  		  if($_REQUEST['temp']=='productos' ){
			  
				if($perm_chkElemProd=='S'){
				   $resukltado=mysql_query("select * from clasificacion where idclasificacion='".$row1['clasificacion']."' and agrupado='".$perm_ElemProd."'");
				   $tepCont=mysql_num_rows($resukltado);
				   //echo "entro";
				    if($tepCont==0){
				    continue;
				    }
        ...
        <tr bgcolor="<?php echo $cfila?>" onmouseover='entrada(this)' >
		 ...
		 if($_REQUEST['temp']=='productos' ){
		 ?>
		  <td width="12%"><font color="#000000" face="Verdana, Arial, Helvetica, sans-serif"  style="font-size:10px; font:bold">
		 <?php
		 ...
		 <?php } ?> 
          ...
          <td width="47%">
		  <font color="#000000" face="Verdana, Arial, Helvetica, sans-serif"  style="font-size:10px; font:bold">
         ...
         <td width="13%" align="center" style="display:none"><?php echo $row1['und']."-".$row1['factor']."-".$row1[$precioxDefecto]."-".$row1['moneda']."-".$row1['series']."-".$row1['serie']."-".$preCostoprod."-".$row1['precio5']."-".$estado_percep."-".$por_percep."-".$dir_clie."-".$row1['kardex']."-".$row1['factorOT']."-".$codbarraEnc."-".$precioMin."-".$codAnexProd."-".$ccajas."-" ?><a name="ancla<?php echo $i;?>" id="ancla"></a></td>
        </tr>
