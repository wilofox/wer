...
Modalbox.Methods = {
	...
	options: {
		...
		overlayOpacity: .25, // Default overlay opacity
	...
	//alert("4");// External hide method to use from external HTML and JS
		if(this.initialized) {
	...
	//alert("19");
		...
		var pagina = self.location.href.match( /\/([^/]+)$/ )[1];
		 		...
		 		//alert(pagina.substr(0,11));
			if(pagina.substr(0,11)=='empresa.php' || pagina.substr(0,19)=='gen_docVendedor.php' ){
					try {
						 if (typeof(eval(document.form_clientes.valor)) != 'undefined'){
						// document.form_series.nroserie[0].focus();
						document.form_clientes.valor.focus();			
						cargar();
						 }else{
						// document.form_series.nroserie.focus();
						document.form_clientes.razonsocial.focus();			
				...
				if(pagina.substr(0,17)=='transferencia.php'){
					var tipomov=2;
				...
				//	alert(pagina.substr(0,11));
					if(pagina.substr(0,11)=='gen_doc.php'){
						document.form_series.scanner.focus();
					}else{
					try {	
						if(document.formulario.tipodoc.value=='V'){
						document.form_series.scanner.focus();
						}else{
						 document.form_clientes.valor.focus();
						 cargar();
		...
		//alert(document.form_series.nroserie.length);
		//document.form_series.nroserie[0].focus();
