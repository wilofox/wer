...
// filtro de agrupacion				
		if ($agruparc==0){
			$SqlRk=" $filtro and ca.tipo='2' and ca.flag!='A' ORDER BY concat(pa.t_pago,ca.cod_ope,ca.serie,ca.Num_doc) ";
		}else{
			$SqlRk=" $filtro and ca.tipo='2' and ca.flag!='A' ORDER BY concat(pa.t_pago,ca.cod_ope,ca.serie,ca.Num_doc) ";
		}
			//echo $SqlRk;		
	...
	while($row=mysql_fetch_array($resultado2)){
		...
		echo" <tr height='30' align='center'><td colspan='13' class='Detalle1' ><strong>TOTAL TIPO PAGO S/.: <span style='color=#0066FF'>".number_format($totalTpagoS,2)."</span></strong></td></tr>";
		
		echo" <tr height='30' align='center'><td colspan='13' class='Detalle1' ><strong>TOTAL TIPO PAGO US$: <span style='color=#0066FF'>".number_format($totalTpagoD,2)."</span></strong></td></tr>";
		...
		if($row['pamon']=='S/.' && $row['patip']!="CANJE" && $row['patip']!="PERCEPCION"){
		...
		$totalTpagoS=number_format($totalTpagoS,2,'.','')+number_format($xmonto,2,'.','');
			...
			if($row['patip']!="CANJE" && $row['patip']!="PERCEPCION"){
				$temp_monto1="";
...
echo 'TOTAL PAGINA (S/.) &nbsp;&nbsp;&nbsp; '.number_format($TolS,2,'.',',').' &nbsp;&nbsp;&nbsp;<br>';
echo 'TOTAL PAGINA (US$.) &nbsp;&nbsp;&nbsp; '.number_format($TolD,2,'.',',').' &nbsp;&nbsp;&nbsp;</b></div></td>
<tr></table>';
	}else{
		echo '<table border="0">
		<tr>
		...
		echo 'TOTAL FECHA (US$.) &nbsp;&nbsp;&nbsp; '.number_format($TolD,2,'.',',').' &nbsp;&nbsp;&nbsp;</b></div></td>
		<tr></table>';
	}
//}
if(!isset($_REQUEST['excel'])){
