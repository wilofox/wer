...
<?php
	...
	if ($agruparc==0){
		$SqlRk=" $filtro group by cliente order by cliente ";
	}else{
		if ($agruparc==1){
			$SqlRk=" $filtro group by LEFT( fecha, 10 ) order by fecha";
		}else{
			$SqlRk=" $filtro group by condicion order by condicion ";
		}
	}
	//echo $SqlRk;		
	...
	$sql2= "select * from cab_mov $SqlRk  LIMIT $inicio, $registros";
			...
			$sql2= "select * from cab_mov $SqlRk ";
		}
	// echo $sql2;
	...
	?>
		...
		<tr>
			...
			<td height="23" colspan="2" align="center" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold"><?=$row3['almacen'];?></td>
			<td width="85" align="center" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold" ><?=$row3['cod_ope'].$row3['Num_doc'];?></td>
			<td width="71" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold"><?=formatobarrafecha(substr($row3['fecha'],0,10));?></td>
			<td width="92" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold">&nbsp;<?=caracteres($row3['flag_r']); ?></td>
			<td width="100" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold"><?=caracteres($row3['nombre']); ?></td>
			<td width="85" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold" title="<?=caracteres($row3['razonsocial']);?>" ><? echo substr(caracteres($row3['razonsocial']), 0, 12)  ?></td>
			<td width="102" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold" title="<?=$row3['usuario'];?>" ><? echo substr(caracteres($row3['usuario']), 0, 12)  ?></td>
			<td colspan="2" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold" align="center"><? if ($row3['incluidoigv']=='S'){echo 'IGV Inclu.';}else{echo 'NO IGV Inclu.';} ?>&nbsp;</td>
			<td width="66" style=" border-bottom:#CCCCCC solid 1px; font-weight:bold" align="right"><? if ($row3['moneda']=='01'){echo'S/.';}else{echo'US$.';}  ?> <?=$row3['total']; ?></td>
		</tr>
		</table>
		<?php 		
		//$TolSoles=0;
		...
		?>
			...
			<tr>   
				...
				<td width="271" title="<?=$row4['nom_prod'];?>" ><? echo substr(caracteres($row4['nom_prod']),0,40) ?></td>
				<td width="74" align="center" ><?php echo number_format($row4['precio'],4)  ?></td>
				<td width="74" align="center" ><?php echo $row4['desc1']  ?></td>
				<td width="74" align="center" ><?php echo $row4['desc2']  ?></td>
