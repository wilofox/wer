...
*/
$controlCantdep='F';
$sql="select * from det_mov where cod_cab='".$row['cod_cab']."'  "; // and flag_r='RO' 
$resultadoX=mysql_query($sql,$cn);
while($rowX=mysql_fetch_array($resultadoX)){
	//if ($rowX['flag_r']=='RO' || $rowX['flag_r']=='RORA'){
		if($rowX['cantdesp']<$rowX['cantidad']){
		$controlCantdep='T';
		}
	//}
}
...
$sql="select * from det_mov where cod_cab='".$row['cod_cab']."' and  cantdesp<cantidad"; // and flag_r='RO' 
$resultadoX=mysql_query($sql,$cn);
$tempCOnt=mysql_num_rows($resultadoX);
//echo "-->".$Estado;
if($tempCOnt==0 && $Estado=='P'){
continue;
}
...
?>
  ...
  <tr bgcolor="<?php echo $color_row?>" onClick="entrada(this);" ondblclick="doc_det(this)" >
	  ...
	  <? } ?>	  </td>
			...
			$contreg=mysql_num_rows($resultadoMS);
			echo $contreg;
