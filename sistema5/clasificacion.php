...
include('conex_inicial.php');
	...
	$strSQL_cla="select * from producto where clasificacion='".$_REQUEST['cod']."' ";
	$resultado_cla=mysql_query($strSQL_cla,$cn);
	$cont=mysql_num_rows($resultado_cla);
	
		if($cont==0){
		$strSQL="delete from clasificacion where idclasificacion='" . $_REQUEST['cod'] . "'";
		mysql_query($strSQL,$cn);
		}else{
		echo "<script>alert('Esta clasificación tiene productos relacionados...')</script>";
...
<form name="form1" method="post" action="clasificacion.php">
  <table width="100%" border="0">
     <tr style="background:url(imagenes/white-top-bottom.gif)">
       <td height="27" colspan="13" style="border:#999999">&nbsp;<span class="Estilo34">Administraci&oacute;n :: Art&iacute;culos :: <span class="Estilo14 Estilo38">Clasificaci&oacute;n
    ...
    <tr>
      <td align="center"><table width="631" border="0" align="center" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" >
        ...
        <tr height="20px" style="background:url(imagenes/sky_blue_grid.gif); border:#999999 solid 1px">
          <td width="92" align="center" style=" border:#CCCCCC solid 1px" class="EstiloTexto1">C&oacute;digo</td>
          <td width="275" align="center" class="EstiloTexto1" style=" border:#CCCCCC solid 1px">Descripci&oacute;n</td>
          <td width="77" align="center" class="EstiloTexto1" style=" border:#CCCCCC solid 1px"><span class="EstiloTexto1" style=" border:#CCCCCC solid 1px">Agrupado</span></td>
...
//echo "select * from clasificacion  $where order by idclasificacion ";
  $resultados = mysql_query("select * from clasificacion  order by idclasificacion ",$cn);
...
while($row=mysql_fetch_array($resultados))
{
 ?>
        <tr  bgcolor="#FFFFFF">
          <td align="center"><span class="Estilo12"><?php echo $row['idclasificacion'];?></span></td>
          <td><span class="Estilo12"><?php echo $row['des_clas'];?>&nbsp;</span></td>
          <td align="center"><span class="Estilo12"><?php echo $row['agrupado'];?></span></td>
          <td width="87" align="center" bgcolor="#FFFFFF"><span class="Estilo13"><span class="Estilo12">&nbsp;<a href="javascript:editar('<?php echo $row['idclasificacion']?>','actualizar');"><img src="imgenes/ico_edit.gif" border="0"></a></span></span></td>
          <td width="84" align="center" bgcolor="#FFFFFF"><span class="Estilo13"><span class="Estilo12"><a href="javascript:eliminar('<?php echo $row['idclasificacion']?>');"><img src="imgenes/eliminar.gif" border="0"></a></span></span></td>
        </tr>
        <?php  
...
function editar(cod,accion){
	window.open('editar_clasificacion.php?cod='+cod+'&accion='+accion,'ventana','height=250, width=400, top=200, left=250, status=yes');
}

function eliminar(cod){
	if(confirm('Esta seguro que desea eliminar este producto?')){
	location.href='clasificacion.php?cod='+cod;
	//this.form1.submit();
	}
