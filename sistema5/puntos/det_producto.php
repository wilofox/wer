...
<div id="resultado" style="width:355px; height:212px; overflow:auto; padding-left:5px;"   >	
	...
	//-------------------------------------------
		...
		$campotienda="saldo".$tienda;
     	...
     	$strSQL="select * from producto  where $criterio like '%$valor%' and canje='S' and $campotienda >0 $filtro2  ";
	$j=0;
...
//list($condicion)=mysql_fetch_array(mysql_query("select nombre from condicion where codigo='".$row['condicion']."'"));
 ?>
       <tr bordercolor="#CCCCCC"  bgcolor="#F9F9F9" onClick="entrada(this);canjear(this)" ondblclick="">
          <td width="20" align="center" ><input style="border: 0px; background:none; " type="radio" name="xaux" value="<?php  echo $row['idproducto']?>" /></td>
          
          <td width="45"><span class="Estilo12"><?php echo $row['idproducto']; ?></span></td>
          <td width="2" ></td>
          <td width="174" ><span class="Estilo12"><?php echo caracteres(substr($row['nombre'], 0, 25)) ?>
            <input type="hidden" name="prodet" value="<?=caracteres($row['nombre']);?>" />
          </span></td>
          <td width="52" align="right"><span class="Estilo12"><?php echo $row['puntos']?>
            <input type="hidden" name="puntos" value="<?=$row['puntos'];?>" />
          </span></td>
          <td width="50" align="right"><?php echo "0.00"; ?>
          <input type="hidden" name="efectivo" value="<?="0.00";?>" /></td>
      </tr>
		...
		if($row['efectivo']!="0" && $row['puntos2']!="0"){
		?>
		 ...
		 <tr bordercolor="#CCCCCC"  bgcolor="#F9F9F9" onClick="entrada(this);canjear(this)" ondblclick="">
          ...
          <td width="52" align="right"><span class="Estilo12"><?php echo $row['puntos2']?>
            <input type="hidden" name="puntos" value="<?=caracteres($row['puntos2']);?>" />
          </span></td>
