...
$sqlx="  select * from modelo  $SQLConsulta ";
...
$strSQL="$sqlx order by fecha desc LIMIT $inicio, $registros";
...
?>
  ...
  <tr bgcolor="<?php echo $color_row?>" onClick="entrada(this)" ondblclick="doc_det(this)"  >
      ...
      <input style="border: 0px; background:none; " type="radio" name="XDato" value="<?=$row['cod_mod'];?>" /><span class="texto2" style="font-size:11px;"><b><? echo $j ?></b></span></td>
      <td width="40" align="center" class="texto1" >
	  <? if ($AnRK<>'3'){ ?>
	   <input 
	   style="border: 0px; background:none; " type="checkbox" name="xcodigo" value="<?=$row['cod_mod'];?>" onclick="Anular('S<?=$AnRK;?>')" />
	  <? } ?></td>
	  <td width="150" class="texto1"><?php echo $row['alias']; ?></td>	
      <td width="178" class="texto1" title="<?php echo $row['nom_prodmodelo'];?>" ><?php echo substr($row['nom_prodmodelo'],0,30);?></td>
             
			  <td width="81" class="texto1" ><?php						
				...
				?></td>	
              <td width="75" class="texto1"><?php	echo formatofecha(substr($row['fecha'],0,10)); ?></td>
              <td width="75"  class="texto1"><?php echo formatofecha(substr($row['f_venc'],0,10)); ?></td>
			  <td width="51" class="texto1"><?php echo $row['items']; ?></td>	
			  <td width="81" class="texto1"><?php echo $row['total'];?></td>
			   <td width="45" class="texto1"><?php
			   if ($row['archivo']<>''){
			   	echo  '<a href="'.$row['archivo'].'" target="_blank"><img src="../imgenes/archivo.jpg"  width="16" height="16" border="0"></a>';
			   }
			   ?></td>
    </tr>
